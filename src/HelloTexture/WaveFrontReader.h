    ...
    HRESULT LoadMTL(_In_z_ const wchar_t* szFileName)
    {
        ...
        for (;; )
        {
            ...
            else if (0 == wcscmp(strCommand.c_str(), L"Ka"))
            {
                ...
                float r, g, b;
                InFile >> r >> g >> b;
                curMaterial->vAmbient = XMFLOAT4(r, g, b, 1);
            }
            else if (0 == wcscmp(strCommand.c_str(), L"Kd"))
            {
                ...
                float r, g, b;
                InFile >> r >> g >> b;
                curMaterial->vDiffuse = XMFLOAT4(r, g, b, 1);
            }
            else if (0 == wcscmp(strCommand.c_str(), L"Ks"))
            {
                ...
                float r, g, b;
                InFile >> r >> g >> b;
                curMaterial->vSpecular = XMFLOAT4(r, g, b, 1);
            }
            else if (0 == wcscmp(strCommand.c_str(), L"Ke"))
            {
                ...
                float r, g, b;
                InFile >> r >> g >> b;
                curMaterial->vEmissive = XMFLOAT4(r, g, b, 1);
            ...
            else if (0 == wcscmp(strCommand.c_str(), L"map_Ka"))
            {
                // Ambient texture
                LoadTexturePath(InFile, curMaterial->strAmbient, MAX_PATH);
            }
            else if (0 == wcscmp(strCommand.c_str(), L"map_d"))
            {
                // Transparency texture
                LoadTexturePath(InFile, curMaterial->strTransparency, MAX_PATH);
            }
            else if (0 == wcscmp(strCommand.c_str(), L"map_bump")
                || 0 == wcscmp(strCommand.c_str(), L"bump"))
            {
                // Bump texture
                LoadTexturePath(InFile, curMaterial->strBump, MAX_PATH);
            }
            else {
                OutputDebugStringW(L"Unknown command: ");
                OutputDebugStringW(strCommand.c_str());
...
#pragma warning( suppress : 4127 )
    ...
    struct Material
    {
        ...
        wchar_t strAmbient[MAX_PATH];
        wchar_t strTransparency[MAX_PATH];
        wchar_t strBump[MAX_PATH];
        wchat_t strDissolve[MAX_PATH];

        Material() noexcept :
            vAmbient(0.2f, 0.2f, 0.2f, 1.0f),
            vDiffuse(0.8f, 0.8f, 0.8f, 1.0f),
            vSpecular(1.0f, 1.0f, 1.0f, 1.0f),
            vEmissive(0.f, 0.f, 0.f, 1.0f),
            nShininess(0),
            fAlpha(1.f),
            bSpecular(false),
            bEmissive(false),
    ...
    int GetVerticesSize() { return vertices.size() * sizeof(Vertex); }
    int GetIndicesSize() { return indices.size() * sizeof(index_t); }
