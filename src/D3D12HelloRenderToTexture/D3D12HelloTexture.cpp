...
void D3D12HelloTexture::InitPresentPass()
{
    ...
    // Create texture buffer
    {
        ...
        m_device->CreateRenderTargetView(m_presentPass_frameDRes.Get(), &rtvDesc, rtvHandle);
    }
...
void D3D12HelloTexture::OnRender()
{
    ...
    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_presentPass_frameDRes.Get(), D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET));

    // Clear render to texture
    CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle_renderFromTex(m_gBufferPass_rtvHeap->GetCPUDescriptorHandleForHeapStart());
    ...
    ResetCommandList();
    m_commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(m_presentPass_frameDRes.Get(), D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));
    SubmitCommandList();
